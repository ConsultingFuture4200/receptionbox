...
**Researched:** 2026-05-04 (initial); **rewritten 2026-05-11 for DR-39 Jetson Orin pivot**
**Researcher:** Project research agent (GSD)
**Overall confidence:** HIGH (single-substrate same-vendor CUDA stack; cross-stack ROCm risk eliminated)

> **Pivot note (2026-05-11):** This document was rewritten in place for the DR-39 ratification. The pre-pivot version (619 lines, ROCm-heavy) is preserved in git history at tag `pivot/strix-halo-end-state` (`4c0bb57`) and on protected branch `archive/amd-rocm-substrate`. The mirror in `CLAUDE.md §Tech Stack` is kept in sync with this document — both reflect the H100-only, Jetson AGX Orin 64GB target post-DR-39.
...
- **Lambda, CoreWeave:** higher friction sign-up, longer minimum commits. Overkill for a <$50 Phase 0.
- **Modal, Replicate, Banana:** serverless wrong abstraction; per-request billing punishes the iterative dev loop. Want persistent dev environment with shell access.
- **Thunder Compute:** cheaper on paper but newer provider with thinner tooling.
- **MI300X / TensorWave / Vultr (AMD ROCm rail):** killed by DR-39. The entire AMD rail was archived at tag `pivot/strix-halo-end-state` and branch `archive/amd-rocm-substrate`. RunPod's MI300X SKU (`AMD Instinct MI300X OAM`, $1.99/GPU-hr secure cloud) is preserved in repo history if ever reactivated.
...
- Three cost-adapter implementations preserved in repo: `cost/adapters/runpod.py` (active), `cost/adapters/vultr.py` (backup, sentinel-guarded), `cost/adapters/tensorwave.py` (stub-with-warning per DR-31 sharing policy v0.1.0).
...
The custom `rbox-pod` image (v8 → v18 history in Phase 02) bakes:
- `tools/pod_entrypoint.sh` as ENTRYPOINT (idempotent `_SHUTDOWN_DONE` guard; reads `BOOTSTRAP_MODE` + `GATE` env vars)
- All 4 HF-pinned models cached on `/models` with revision-pinned paths
- `runpod` SDK 1.9.0 + cuda extras via `pyproject.toml` `[project.optional-dependencies]` cuda group
- `corpus_500` and other benchmark assets
...
Phase 02 inherited this discipline; `bench/images.lock.yaml` carries the active `rbox-pod` digest with `captured_utc` and base-image traceability.
...
- **TensorRT-LLM on Orin (out of Phase 0 scope, into Phase 1 capacity planning):** NVIDIA's recommended inference path on JetPack 6; ~1.5–2× faster than Ollama on Orin per Jetson AI Lab benchmarks. Stay on vLLM for Phase 0 measurement to keep the H100 → Orin substrate apples-to-apples; revisit TensorRT-LLM as a Phase 1 optimization.
- SGLang on CUDA is improving but not as battle-tested as vLLM. Stay on vLLM.
- MLC is interesting but its CUDA optimization story is rougher than vLLM's for serving workloads.
...
vLLM serving call with xgrammar backend: pass `--guided-decoding-backend xgrammar` at serve time; clients send JSON-schema-shaped grammars per request via the OpenAI-compatible API. Constrained-label and constrained-JSON paths both supported.
...
Standard phone-path simulation:

```bash
ffmpeg -i input.wav -ar 8000 -ac 1 -c:a pcm_mulaw out.ulaw
ffmpeg -i out.ulaw -ar 16000 -ac 1 reconstructed.wav
```
...
Phase 01 pinned `jiwer >=4.0,<5.0` with `whisper-normalizer` as a separate dep (Pitfall A: STACK.md references to jiwer 3.x are historical — the running code uses 4.x).
...
**Pre-DR-39 risk that is now moot:** the devnen/Chatterbox-TTS-Server ROCm fork had unresolved install issues (devnen issues #92, #192, #445). The CUDA mainline path is well-trodden and has no fork-hunting required.
...
**Pre-DR-39 fork that is now moot:** `moritzchow/Kokoro-FastAPI-ROCm` (AMD GPU PyTorch support). The mainline `remsky/Kokoro-FastAPI` runs cleanly on CUDA.

### §5.3 First-Audio Latency Measurement

Render the same 30 utterances at known load. Measure timestamp from request received → first audio byte emitted. Report warm-path (cached weights) and cold-path (first call after model load) separately. Loudness-normalize via `pyloudnorm` for the A/B downstream test.

### §5.4 TTS Quality A/B (G7)

- Render the same 30 utterances via Chatterbox cloned voice and Kokoro neutral voice (matched normalization; matched sample rate; matched loudness).
...
Same vendor (NVIDIA), same CUDA stack (12.x), same arithmetic — the derate is a numerical ratio, not a cross-stack risk.
...
- For Qwen3-4B short prompts (typical intake exchange, ~200 tokens), this is the dominant factor in TTFT. A 50 ms TTFT on H100 implies ~1.5 s on Orin under FP16 — which exceeds the 900 ms aggregate target.
- **Mitigation:** use INT8 (W8A8 or AWQ-Int4) on Orin to claw back ~4× via INT8 tensor cores. Orin has 275 sparse TOPS INT8; H100 has 3957 — INT8 ratio ~14× rather than 31×.
- The synthesis report MUST measure with the same quantization on both sides (or otherwise show its work in the substitution).
...
- Add an **Ollama overhead factor (~1.3–1.5×)** to LLM stages, since production runtime on Orin will use Ollama/llama.cpp rather than vLLM. (See §3.1 caveat.)
...
| **Anthropic Claude API as cloud LLM in Phase 0** | Phase 0 measures the *local-only* path (PRD FR-R49 OFF default; cloud LLM fallback explicitly out of scope per PROJECT.md). | Skip entirely; not relevant to the gate package |
...
# uv pip install vllm (CUDA wheel, no [rocm] extra), faster-whisper, livekit-agents,
# jiwer, silero-vad, pyloudnorm, etc.
```
...
### Archived (pre-DR-39, no longer authoritative; preserved at git tag `pivot/strix-halo-end-state`)
- TensorWave / Vultr MI300X pricing — see archived branch
- ROCm Docker images (rocm/pytorch, rocm/vllm) — see archived branch
- AMD vLLM MI300X recipe — see archived branch
- Strix Halo (Ryzen AI Max+ 395) ROCm benchmarks (Phoronix) — see archived branch
- Chatterbox ROCm fork (devnen) — see archived branch
- Kokoro ROCm fork (moritzchow) — see archived branch
- insanely-fast-whisper-rocm — see archived branch
- gfx1151 op-coverage research — see archived branch
