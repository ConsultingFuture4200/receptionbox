...
| T4 — `corpus_500` in image | Un-ignore `corpus_500` in `.dockerignore` | Delivered. Image audit (`results/smoke_pre_v19a/1778368799.audit.json`, archived from `results/smoke/` on 2026-05-12 ahead of v19a image cutover) reports found_count=500 (expected_count=755 — `corpus_g711` and `corpus_hesitation` still excluded by design, smoke needs only `corpus_500`). Image-size delta ≈ +150 MB compressed. |
